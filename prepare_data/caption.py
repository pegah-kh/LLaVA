...
class COCOCaptionFineTuneDataset(Dataset):
    def __init__(self, split='karpathy_train', raw_dataset=None, rank=-1, topk=-1, verbose=True, mode='train', 
        data_dir='/data/mshukor/data', black_image=False, image_size=224, use_data_augmentation=True):
        super().__init__()
        ...
        self.train_transform = transforms.Compose([                        
                transforms.RandomResizedCrop(image_size,scale=(0.5, 1.0), interpolation=Image.BICUBIC),
...
class CCDataset(Dataset):
    def __init__(self, split='CC', raw_dataset=None, rank=-1, topk=-1, verbose=True, mode='train', 
        data_dir='/data/mshukor/data', config_dir='/data/mshukor/data/cc3m.json', max_words=30, 
        image_size=224, use_data_augmentation=True, data_json_dir=None):
        super().__init__()
...
class NoCapsCaptionFineTuneDataset(Dataset):
    def __init__(self, split='val', raw_dataset=None, rank=-1, topk=-1, verbose=True, mode='train', 
        data_dir='/data/mshukor/data/nocaps', black_image=False, image_size=224, use_data_augmentation=True,):
        super().__init__()
...
def get_loader(split='train', mode='train',
               batch_size=32, workers=4, distributed=False, gpu=0,
               topk=-1, data_dir='/data/mshukor/data', local_rank=None, world_size=None, verbose=False, 
               config_dir=None, black_image=False, image_size=224, use_data_augmentation=True, data_json_dir=None, metrics=None):



    if 'CC' in split:
        dataset = CCDataset(split, data_dir=data_dir, mode=mode, topk=topk, verbose=verbose, 
                            rank=gpu, config_dir=config_dir, image_size=image_size, use_data_augmentation=use_data_augmentation, 
...
class COCOCaptionEvaluator:
    def __init__(self, metrics=None):
        ...
        if metrics is not None:
            self.evaluator = language_evaluation.CocoEvaluator(verbose=False, coco_types=metrics) # coco_types=["BLEU", "METEOR", "ROUGE_L", "CIDEr", "SPICE"]
        else:
            self.evaluator = language_evaluation.CocoEvaluator(verbose=False) # coco_types=["BLEU", "METEOR", "ROUGE_L", "CIDEr", "SPICE"]
